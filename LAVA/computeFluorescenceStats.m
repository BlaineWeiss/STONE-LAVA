function [maxftratio, indmax, LowerV] = computeFluorescenceStats(EndfeetonChannelMask)
...
EndfeetonChannelMask(EndfeetonChannelMask == 0) = NaN;
endftmean = mean(EndfeetonChannelMask, 1, 'omitnan');
if all(isnan(endftmean))
    maxftratio = NaN;
    indmax = NaN;
    LowerV = NaN;
    return;
...
LowVals = endftmean(endftmean <= prctile(endftmean, 5));
LowerV = mean(LowVals, 'omitnan');
[maxVal, indmax] = max(endftmean);
maxftratio = 100 * maxVal / LowerV;
end
